...
struct FileMem
{
    ...
    FileMem(const uint8_t* data, size_t size)
        : m_data(data),
          m_size(size)
    {
...
Buzzic2* Buzzic2Load(const uint8_t* data, uint32_t size)
{
    ...
    if (buz->Load(*fh, FileMem(data += sizeof(FileHeader), size - sizeof(FileHeader))))
        return buz;
    delete buz;
    return nullptr;
}
...
bool Buzzic2::Load(const FileHeader& fh, FileMem f)
{
    ...
    memcpy(g_bytePatterns, pat, min<int>(maxPatterns, MAX_PATTERNS) * 16);
    for (int i = 0; i < min<int>(maxSequences, MAX_SEQUENCES); i++)
        for (int k = 0; k < min<int>(maxTemplMelodyLen, MAX_TEMPLATE_MELODY_LEN); k++)
    ...
    while (remainingSamples)
    {
        ...
        if (data)
        {
            ...
            for (uint32_t i = 0; i < samplesToCopy; i++)
                data->left *= masterVolume;
                data->right *= masterVolume;
                data++;
        }
